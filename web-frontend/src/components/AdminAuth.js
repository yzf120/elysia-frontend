  ...
  async handleSendCode() {
    ...
    try {
      ...
      if (this.currentTab === 'register') {
        await adminAPI.sendRegisterCode(phoneNumber);
      } else {
        await adminAPI.sendLoginCode(phoneNumber);
      }
  ...
  async handleSubmit(formData) {
    ...
    try {
      ...
      if (this.currentTab === 'password') {
        ...
        result = await adminAPI.loginWithPassword(phoneNumber, password);
      } else if (this.currentTab === 'sms') {
        const phoneNumber = formData.get('phone_number');
        const code = formData.get('code');
        result = await adminAPI.loginWithSMS(phoneNumber, code);
      } else {
        const phoneNumber = formData.get('phone_number');
        const code = formData.get('code');
        result = await adminAPI.registerWithSMS(phoneNumber, code);
      }
